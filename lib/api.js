...
function createApi(browser) {
    ...
    api.get('/pdf', async function ({ query: { url, } }, reply) {
        if (url && url_1.parse(url)) {
            try {
                const buffer = await print_page_1.printUrl(url, browser);
    ...
    api.post('/pdf', bodyParser.text({ limit: '1mb', type: 'text/html' }), async function ({ body }, reply) {
